...
df = pd.read_csv("bio_feat_time_4pois_pid26clean.csv")
...
for POI in [1,2]:
    target_POI = POI
    min_data = min(len(df[df["POI#"] == target_POI]), len(df[df["POI#"] != target_POI]))
    print("The Min of ", len(df[df["POI#"] == target_POI]), " and ", len(df[df["POI#"] != target_POI]), " is ", min_data)
